...
dirs.lib = path.join(dirs.app, '.webpack', 'lib');
...
var coreJsVersion = JSON.parse(fs.readFileSync(dirs.app + '/node_modules/core-js/package.json')).version;
...
if (!fs.existsSync(path.join(dirs.lib, targetFile))) {
  ...
  cd(path.join(dirs.app, 'node_modules/core-js'));
  exec('npm install');
  cd(dirs.app);
